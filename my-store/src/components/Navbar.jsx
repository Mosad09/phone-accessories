...
function Navbar({
  ...
  const languageLabel = i18n.language === "ar" ? "EN" : "عربي";

  const renderLanguageToggle = (variant = "desktop") => (
    <button
      type="button"
      className={variant === "utility" ? "navbar-utility-pill" : "cart-btn-nav border-0 shadow-none px-2"}
      onClick={toggleLanguage}
      aria-label="Switch Language"
    >
      <span className="fw-bold navbar-lang-label">{languageLabel}</span>
    </button>
  );
  ...
  const renderThemeToggle = (variant = "desktop") => (
    <button
      ...
      className={
        variant === "utility"
          ? "navbar-utility-pill"
          : "cart-btn-nav theme-toggle-btn border-0 shadow-none"
      }
      onClick={toggleTheme}
      aria-label={`Switch to ${theme === "dark" ? "light" : "dark"} mode`}
      aria-pressed={theme === "dark"}
    >
      <i className={`bi ${theme === "dark" ? "bi-sun" : "bi-moon"}${variant === "utility" ? "" : " fs-5"}`} aria-hidden="true"></i>
    </button>
  );
  ...
  return (
    <nav className="navbar navbar-custom py-3" aria-label="Primary navigation">
      <div className="container navbar-shell">
        ...
        <div className="navbar-header-row">
          ...
          >
            ...
            <span className="navbar-brand-name">
              {t("navbar.brand_vel")}
              <span className="text-primary-custom">{t("navbar.brand_trix")}</span>
            </span>
          </a>
          ...
          <div className="navbar-header-actions">
            <button
              type="button"
              className="cart-btn-nav navbar-search-jump"
              onClick={focusSearch}
              aria-label={t("navbar.focus_search")}
            >
              <i className="bi bi-search fs-5" aria-hidden="true"></i>
            </button>
            ...
            >
              ...
              {wishlistCount > 0 && (
                <span className="cart-badge bg-danger">{wishlistCount}</span>
              )}
            </button>
            ...
            >
              ...
              {cartCount > 0 && (
                <span className={`cart-badge ${cartPulse ? "pulse-anim" : ""}`}>{cartCount}</span>
              )}
            </button>
        ...
        {/* Row 2 — Utility bar (mobile) */}
        {isMobile && (
          <div className="navbar-utility-bar">
            {activeRoute === "home" && (
              <button
                type="button"
                className="navbar-utility-pill"
                onClick={onToggleFilters}
                aria-label="Open filters"
              >
                <i className="bi bi-funnel" aria-hidden="true"></i>
                <span className="navbar-utility-pill-text">{t("navbar.filters")}</span>
              </button>
            )}
            {renderLanguageToggle("utility")}
            {renderThemeToggle("utility")}
          </div>
        )}

        {/* Row 3 — Search */}
        <div className="search-wrapper navbar-search navbar-search-row" ref={wrapperRef}>
          {renderSearchField()}
        </div>
      </div>
    </nav>
